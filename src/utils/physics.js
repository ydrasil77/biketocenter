 ...
 *
 * Power balance: P = (Fdrag + Froll) × v
 *   Fdrag = 0.5 × CdA × ρ × v²
 *   Froll = Crr × m × g
 *
 * Gender multiplier: Female riders get 1.15× effective watts
 * (normalises W/kg against typically heavier male riders)
 *
 ...
 */
export function calcSpeed(watts, weightKg, gender, trafficState) {
    if (trafficState === 'RED' || watts <= 0) return 0;
    ...
    for (let i = 0; i < 50; i++) {
        ...
        const Froll = Crr * m * g;
        const Pv = (Fdrag + Froll) * v;
        const dPdv = 3 * 0.5 * CdA * rho * v * v + Froll;
        const dv = (Pv - P) / dPdv;
        v -= dv;
        if (v < 0) { v = 0; break; }
